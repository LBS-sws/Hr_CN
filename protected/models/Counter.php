...
//加班审核(部門)
    public static function getWorkOne() {
        ...
        $count = Yii::app()->db->createCommand()->select("count(a.id)")->from("hr_employee_work a")
            ...
            ->where("hb.user_id != '$staff_id' and a.status in (1) AND a.z_index =1 AND b.city='{$city}' AND b.department=:department",array(":department"=>$department))->queryScalar();
        return $count;
    }
//请假审核(部門)
...
//指定出差审核(審核)
    public static function getAppointTrip() {
        $count = 0;
        if(Yii::app()->user->validRWFunction('ZG13')){
            $uid = Yii::app()->user->id;
            $auditSql = "";
            ...
            $count = Yii::app()->db->createCommand()->select("count(a.id)")
                ...
                ->where(" a.status in (1) and ({$auditSql})")->queryScalar();
        }
        return $count;
    }
//出差审核(審核)
    public static function getTripAudit() {
        ...
        $count = Yii::app()->db->createCommand()->select("count(a.id)")->from("hr_employee_trip a")
            ->leftJoin("hr_employee b","a.employee_id = b.id")
            ->where("a.status in (1) AND a.z_index=4 AND b.city in ($city_allow)")->queryScalar();
        return $count;
    }
//出差申请（出差结果)
